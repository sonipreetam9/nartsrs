				...
				<div class="panel panel-default">
					...
					<div class="panel-body table-responsive">
						...
						if (isset($_POST["Deletee"])) {
							$bill = $_POST['id'];
							// $sqll = "UPDATE  SET verify='1' WHERE id=$bill";
							$sqll = "DELETE FROM tbl_student WHERE id=$bill";
							$result = mysqli_query($link, $sqll);
							if (mysqli_affected_rows($link)) {
						?>
								<div class="alert alert-success bg-success text-white" role="alert">Deleted</div>
							<?php
								header('Location: ' . $_SERVER['PHP_SELF']);
							} else {
							?>
							...
							<tbody>
								...
								?>
									<tr>
										...
										<td>
											<a href="student_details.php?id=<?= $row['id']; ?>" class="btn btn-icon btn-xs btn-info mr-1">
												view Details
											</a>
											<a href="student_update.php?id=<?= $row['id']; ?>" class="btn btn-icon btn-xs btn-warning mr-1">
												Update Details
											</a>
											<?php
											if ($row['Verify'] == "0") {
											...
											<a data-toggle="modal" data-target="#Delete<?= $row['id']; ?>" class="btn btn-icon btn-xs btn-danger mr-1">
												Delete
											</a>
											<div class="modal fade modalbox" id="Delete<?= $row['id']; ?>" tabindex="-1" role="dialog" style="display: none;">
												<div class="modal-dialog">
													<div class="modal-content">
														<div class="modal-header">
															<button type="button" class="close" data-dismiss="modal">&times;</button>
														...
														<div class="modal-body">
															<form method="post" enctype="multipart/form-data">
																...
																<div class="row">
																	<div class="col-md-12 col-xs-12">
																		<input type="submit" class="btn btn-danger" name="Deletee" value="Delete">
																		<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
																	</div>
																</div>
															</form>
